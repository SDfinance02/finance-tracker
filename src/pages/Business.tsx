...
export function Business(){
  ...
  return <>
    <PageHeader title="My BV" subtitle="Belgian company cockpit · management view, not a substitute for statutory accounts or your accountant." actions={<><select className="select compact-select" value={activeEntity.id} onChange={e=>setEntityId(Number(e.target.value))}>{entities.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select><button className="btn" onClick={editEntity}><Settings2 size={14}/>Company</button><button className="btn primary" onClick={()=>{setTxForm(blankTx());setTxOpen(true)}}><Plus size={14}/>Entry</button></>}/>
